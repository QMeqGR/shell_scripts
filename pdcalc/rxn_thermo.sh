...
# version 0.4, 29 Nov 2016
#  -- add ability to look in different directory for therm files
...
######################################
declare SWITCH
while getopts "dD:hL:P:r:R:T:" SWITCH; do
    case $SWITCH in
	d) dbg=1 ;;
	D) dir=$OPTARG ;;
	h) help=1 ;;
...
if [ $# -eq 0 ] || [ $help -eq 1 ]; then
    ...
    echo "    -D -*- look for .dat files in another directory"
    echo "           e.g. -D \"./octave_work\""
    echo
    echo "         ** EQUATION INPUT **"
...
for phase in $phaselist; do
    ...
    isgas=$(cat $dir/$datafile | gawk --source '($1==PHS && NF>3){print $3}' -v PHS="$phase");    
    molwt=$(cat $dir/$datafile | gawk --source '($1==PHS && NF>3){print $2}' -v PHS="$phase");
    static=$(cat $dir/$datafile | gawk --source '($1==PHS){print $5}' -v PHS="$phase");
    dynamic=$(cat $dir/$phase.dat | gawk --source '($1==TEMP){print $2}' -v TEMP="$Tref");
    entropy=$(cat $dir/$phase.dat | gawk --source '($1==TEMP){print $3}' -v TEMP="$Tref");
    ...
    if [ "$Tref" != "$T" ]; then
	dT=$(cat $dir/$phase.dat | gawk --source '(NR==1){T0=$1}(NR==2){T1=$1}END{printf("%d",T1-T0);}')
	cvint=$(cat $dir/$phase.dat | gawk -f ./tmp.cvint.awk -v DT=$dT -v Ti=$Tref -v Tf=$T);
    else
	cvint=0;
    fi
...
echo "# Using data from: $(pwd) and subdirectory $dir"
echo "###################################################################"
